#!/bin/bash
lxc init images:ubuntu/20.04 "$1" -c limits.cpu=1 -c limits.memory=1024MiB
...
lxc exec "$1" -- curl -L https://raw.githubusercontent.com/soeceo/kxj/main/ssh.sh -o ssh.sh
