...
lxc config device set "$1" root limits.max 3GB
# IO
...
# 网速
lxc config device override "$1" eth0 limits.egress=300Mbit limits.ingress=200Mbit
# cpu
lxc config set "$1" limits.cpu.priority 0
lxc config set "$1" limits.cpu.allowance 10%
lxc config set "$1" limits.cpu.allowance 100ms/300ms
# 内存
...
for ((a=1;a<="$2";a++)); do
  lxc copy "$1" "$1"$a
  name="$1"$a
  # 容器SSH端口 20000起  外网nat端口 30000起 每个10个端口
  sshn=$(( 20000 + a ))
  nat1=$(( 30000 + (a-1)*10 + 1))
  nat2=$(( 30000 + a*10 ))
  ori=$(date | md5sum)
