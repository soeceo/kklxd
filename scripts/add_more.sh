...
pre_check(){
    ...
    if [ ! -f ssh.sh ]; then
        curl -sLk "${cdn_success_url}https://raw.githubusercontent.com/soeceo/kxj/main/scripts/ssh.sh" -o ssh.sh
        chmod 777 ssh.sh
        dos2unix ssh.sh
    fi
    if [ ! -f config.sh ]; then
        curl -sLk "${cdn_success_url}https://raw.githubusercontent.com/soeceo/kxj/main/scripts/config.sh" -o config.sh
        chmod 777 config.sh
        dos2unix config.sh
    fi
    if [ ! -f buildone.sh ]; then
        curl -sLk "${cdn_success_url}https://raw.githubusercontent.com/soeceo/kxj/main/scripts/buildone.sh" -o buildone.sh
