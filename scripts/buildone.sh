...
if echo "$system" | grep -qiE "alpine"; then
    if [ ! -f /usr/local/bin/alpinessh.sh ]; then
        curl -L https://raw.githubusercontent.com/soeceo/kxj/main/scripts/alpinessh.sh -o /usr/local/bin/alpinessh.sh
    ...
    if [ ! -f /usr/local/bin/ssh.sh ]; then
        curl -L https://raw.githubusercontent.com/soeceo/kxj/main/scripts/ssh.sh -o /usr/local/bin/ssh.sh
    ...
    if [ ! -f /usr/local/bin/config.sh ]; then
        curl -L https://raw.githubusercontent.com/soeceo/kxj/main/scripts/config.sh -o /usr/local/bin/config.sh
...
if [ -n "$9" ]; then
  if [ "$9" == "Y" ]; then
    if [ ! -f "./build_ipv6_network.sh" ]; then
      ...
      curl -L https://raw.githubusercontent.com/soeceo/kxj/main/scripts/build_ipv6_network.sh -o build_ipv6_network.sh && chmod +x build_ipv6_network.sh > /dev/null 2>&1
    fi
